...
select a.team,
sum(case when ae.medal='Silver' then 1 else 0 end) as total_silver_medals,
...
with cte as (select ae.year,a.name,
count(1) as no_gold_medal
from athlete_events ae
inner join athletes a on ae.athlete_id=a.id
where medal='Gold'
group by ae.year,a.name)
select year,no_gold_medal,
string_agg(name,',') as players 
from(
select *,
rank() over(partition by year order by no_gold_medal desc) as rn
from cte ) a
where rn=1
group by year,no_gold_medal;
